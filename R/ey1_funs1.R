...
############## TMLE targeting EY1
#' @export
ey1_update1 <- function(tmledata, Q.trunc = 0.001, ...) {
...
#' @export
ey1_estimate1 <- function(tmledata, ...) {
...
#' @export
ey1_update2.1 <- function(tmledata, Q.trunc = 0.001, ...) {
    eps_q <- 0
    # fluctuate Q
...
#' @export
ey1_estimate2.1 <- function(tmledata, ...) {
    ...
    # influence curves
    Dstar_psi1 <- with(tmledata, HA * (Y - QAk))
    Dstar_psi2 <- with(tmledata, Q1k - psi)
    
    list(tmledata = tmledata, ests = c(psi = psi), Dstar = list(Dstar_psi1 = Dstar_psi1,
                                                                Dstar_psi2 = Dstar_psi2))
